...
public class IntentIntegrator {
	private static final String NOTES_PACKAGE_NAME = "com.threebanana.notes"; 
        private static final String NOTES_GOOGLE_ANALYTICS = "&referrer=utm_source%3Dcatchhealth%26utm_medium%3Dapp%26utm_campaign%3Dinstall112210";
	private static final String NOTES_MARKET_URI = "http://market.android.com/search?q=pname:" + NOTES_PACKAGE_NAME + NOTES_GOOGLE_ANALYTICS;
	
	private static final int NOTES_MIN_VERSION_CODE = 45;
	...
	public void createNote(String message) {
		createNote(message, null, -1, false, null, false);
	}

	public void createNote(String message, int cursorPosition) {
		createNote(message, null, cursorPosition, false, null, false);
	}

	public void createNote(String message, boolean autoSave) {
		createNote(message, null, -1, autoSave, null, false);
	}
	
	public void createNote(String message, boolean autoSave, boolean isFromWidget) {
		createNote(message, null, -1, autoSave, null, isFromWidget);
	}

	public void createNote(String message, Uri imageUri) {
		createNote(message, null, -1, false, imageUri, false);
	}
	
	public void createNote(String message, Location location) {
		createNote(message, location, -1, false, null, false);
	}
	
	public void createNote(String message, Location location, int cursorPosition, boolean autoSave, Uri imageUri, boolean isFromWidget) {
		...
		// a String.
		intent.putExtra(Intent.EXTRA_TITLE, "Catch Health Tracker");
	...
	public boolean isNotesInstalled() {
		...
		try {
			_context.getPackageManager().getPackageInfo(NOTES_PACKAGE_NAME, PackageManager.GET_ACTIVITIES);
	...
	public boolean isNoteFitsRequirement() {
		// Verify that correct version of notes is installed
		try {
	...
	//Widget version, just so it doesn't crash on widget
	public boolean isNotesInstalledWidget() {
		// Verify that correct version of notes is installed
		try {
			PackageInfo packageInfo = _context.getPackageManager().getPackageInfo(NOTES_PACKAGE_NAME, PackageManager.GET_ACTIVITIES);
	...
	public void displayNotesMarketPageWidget() {
		try {
			Uri uri = Uri.parse(NOTES_MARKET_URI);
			Intent intent = new Intent(Intent.ACTION_VIEW, uri);
			intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
